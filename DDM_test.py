...
if __name__ == "__main__":
    ...
    for istep,  val_data in enumerate(test_loader):
        ...
        visuals = diffusion.get_current_data_ddpm_only()
        # defm_frames = visuals['deform'].squeeze().numpy().transpose(0, 2, 3, 1)[1:]
        # from IPython import embed; embed()
        batch = visuals['code'].shape[0]
        for i in range(0,batch):
            print(i)
            assert visuals['code'].shape[1] == 1
            code_frames = visuals['code'][i,0,:,:,:].numpy().transpose(1, 2, 0) # WARNING:::: moving the batch to the end...  but should be doing this PER image (loop it! )
            ...
            if save_to_nifti:
                ...
                code_frameo = nib.Nifti1Image(code_frames,np.eye(4))
                ...
                nib.save(code_frameo,os.path.join(result_path, '%s_denoised.nii.gz' % (dataName)))
